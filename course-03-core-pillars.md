...
>
> **课程导语**：课程一是让你看见 Agent，课程二是让你理解 Agent 的核心能力演进，课程三是让你动手实现。本课程的五大支柱是后续所有架构设计的基础——没有支柱，架构就是空中楼阁。
>
...
> **本课小结**：Memory 是 Agent 五大支柱中解决"遗忘"问题的核心能力。短期记忆管理让 Agent 在长对话中不丢失关键信息；长期记忆让 Agent 不再是"金鱼记忆"，能跨会话积累用户偏好和经验。课程四的 Memory 章节将在此基础上，深入向量数据库选型、MemGPT 虚拟内存思想、记忆衰减策略等架构话题。
...
**ReAct 循环（图中外层）** 则是运行时引擎。它不解决某个单一的局限，而是将五大支柱串联为 Thought → Action → Observation 的决策循环——LLM 思考后决定调用哪个工具、检索什么知识、读写哪条记忆，观察结果后再进入下一轮思考。五大支柱是"能力"，ReAct 是让这些能力协同工作的"机制"。
