...
- [第二章：RAG / 外部知识接入](#第二章rag--外部知识接入)
  ...
  - [2.2 技术演进背景：从"开卷考试"到 RAG](#22-技术演进背景从开卷考试到-rag)
  - [2.3 对问题的思考：知识从哪里来，如何进入决策](#23-对问题的思考知识从哪里来如何进入决策)
  - [2.4 解决方案：外部知识接入链路](#24-解决方案外部知识接入链路)
  - [2.5 迭代路径：从手动上下文到可信知识系统](#25-迭代路径从手动上下文到可信知识系统)
  - [2.6 常见失败与修正](#26-常见失败与修正)
  - [2.7 判断边界：什么时候不需要外部知识接入](#27-判断边界什么时候不需要外部知识接入)
- [第三章：Memory：状态延续能力](#第三章memory状态延续能力)
  - [3.1 问题代入：Agent 总是忘记、重复或带错历史](#31-问题代入agent-总是忘记重复或带错历史)
  - [3.2 技术演进背景：ChatGPT 的记忆之路](#32-技术演进背景chatgpt-的记忆之路)
  - [3.3 对问题的思考：不是所有历史都应该变成记忆](#33-对问题的思考不是所有历史都应该变成记忆)
  - [3.4 解决方案：Memory 的读写更新机制](#34-解决方案memory-的读写更新机制)
  - [3.5 迭代路径：从会话状态到长期记忆](#35-迭代路径从会话状态到长期记忆)
  - [3.6 常见失败与修正](#36-常见失败与修正)
  - [3.7 判断边界：什么时候不需要 Memory](#37-判断边界什么时候不需要-memory)
- [第四章：Planning / Workflow Patterns](#第四章planning--workflow-patterns)
  - [4.1 问题代入：任务变长后 Agent 开始漂移](#41-问题代入任务变长后-agent-开始漂移)
  - [4.2 技术演进背景：从 ReAct 到 LangGraph](#42-技术演进背景从-react-到-langgraph)
  - [4.3 对问题的思考：任务组织比单步决策更重要](#43-对问题的思考任务组织比单步决策更重要)
  - [4.4 解决方案：常见任务组织模式](#44-解决方案常见任务组织模式)
  - [4.5 迭代路径：从固定步骤到可回放图结构](#45-迭代路径从固定步骤到可回放图结构)
  - [4.6 常见失败与修正](#46-常见失败与修正)
  - [4.7 判断边界：什么时候不需要 Planning](#47-判断边界什么时候不需要-planning)
- [第五章：Reflection：失败修正能力](#第五章reflection失败修正能力)
  - [5.1 问题代入：Agent 明明错了，却继续往下走](#51-问题代入agent-明明错了却继续往下走)
  - [5.2 技术演进背景：Self-Refine 和 Reflexion](#52-技术演进背景self-refine-和-reflexion)
  - [5.3 对问题的思考：反思必须依赖反馈信号](#53-对问题的思考反思必须依赖反馈信号)
  - [5.4 解决方案：触发、批判、修正、停止](#54-解决方案触发批判修正停止)
  - [5.5 迭代路径：从格式校验到基于测试的修正](#55-迭代路径从格式校验到基于测试的修正)
  - [5.6 常见失败与修正](#56-常见失败与修正)
  - [5.7 判断边界：什么时候不需要 Reflection](#57-判断边界什么时候不需要-reflection)
- [第六章：Multi-Agent：角色分工能力](#第六章multi-agent角色分工能力)
  - [6.1 问题代入：单个 Agent 的角色开始过载](#61-问题代入单个-agent-的角色开始过载)
  - [6.2 技术演进背景：AutoGen、CrewAI 和"多智能体"的诱惑](#62-技术演进背景autogencrewai-和多智能体的诱惑)
  - [6.3 对问题的思考：多 Agent 不是多写几个角色 Prompt](#63-对问题的思考多-agent-不是多写几个角色-prompt)
  - [6.4 解决方案：分工、并行、互审和协调](#64-解决方案分工并行互审和协调)
  - [6.5 迭代路径：从 Reviewer 到 Supervisor](#65-迭代路径从-reviewer-到-supervisor)
  - [6.6 常见失败与修正](#66-常见失败与修正)
  - [6.7 判断边界：什么时候不需要 Multi-Agent](#67-判断边界什么时候不需要-multi-agent)
...
1.1 里的知识助手 Agent，最终集齐了五种能力。但这不是一次加完的——它是在几个月的使用中，一个崩溃接一个崩溃地被逼出来的。
...
不要因为某个能力在社区里很热门就加入系统。每个能力进入系统之前，都应该有失败证据和评测方式。如果你说不出"没有 RAG 时用户具体在哪一步遇到了什么问题"，那 RAG 就不该是你的第一个优先。
...
课程五解决的是"这个场景该不该加 RAG / Memory / Planning / Reflection / Multi-Agent"。

但当你真正开始在代码里引入这些能力时，你会撞上一类全新问题——不是"该不该加"，而是"怎么加才不失控"：

- 外部知识、Memory 和工具结果同时涌向 LLM，**上下文怎么编排**才不让模型注意力被淹没？
- 长任务中断后恢复，**哪些状态需要持久化、哪些需要丢弃**？
- 多步执行和多 Agent 协作，**每一轮的输入输出、决策依据、错误信息如何被完整 Trace**？
- Reflection 的失败修正和 Multi-Agent 的审查结论，**如何进入评测体系**而不是靠感觉判断好坏？
- 权限校验、Prompt Injection 防护、敏感信息过滤，**如何嵌入运行时而不是依赖模型自觉**？

这些问题有一个共同特点：它们不是"增强能力"层面的事，而是**运行时基础设施**层面的事——Context Engineering、Checkpoint、Observability、Evaluation、Guardrails。

课程六（Harness 运行时架构）会承接本课的所有增强能力，把它们装进一个可配置、可追踪、可恢复、可评测、可治理的工程骨架里。换句话说：课程五教你判断"该装什么零件"，课程六教你设计"承载这些零件的底盘"。
