...
**本课的建议**：先用原子工具理解机制，跑通闭环。当你发现某个工具组合在多个任务中反复出现、步骤已经稳定、失败模式也清楚了，再把它沉淀成 Skill（第八章）。
...
这种方式适合工具数量少、工具边界清楚、调用失败代价低的场景。比如一个学习型 Agent 只有读取文件、搜索笔记、计算三个工具，直接让模型选择通常够用。

它的问题也很明显：一旦工具数量上来，模型面对的不是"一个清晰选择"，而是一组相互重叠的描述。`search_docs`、`search_web`、`search_code`、`search_ticket` 看起来都像搜索，如果工具描述没有明确边界，模型就会在相似工具之间摇摆。
...
规则路由适合流程稳定、高风险、可预测的场景。例如查订单状态必须走订单系统，退款必须经过权限和风控检查，删除数据必须先确认。这里不应该让模型自由决定"要不要调用工具"，而应该由系统先把动作边界定住。

但规则路由也有代价：它不擅长处理开放任务。如果用户说"帮我看看这个项目为什么启动失败"，可能需要先读 README，再搜索错误日志，再检查 package 配置，再决定是否运行测试。这个过程很难用一组静态规则完全写死。
...
```text
系统负责边界：当前阶段允许看见哪些工具？
模型负责局部判断：在这些候选工具里，下一步该用哪个？
Runtime 负责兜底：参数是否合法、权限是否通过、结果如何回填？
```

这个拆分很重要。模型擅长在语义上下文中做局部判断，例如"用户给了文件名，所以现在应该读文件"。系统更擅长做确定性约束，例如"当前用户没有写权限，所以不能暴露写文件工具"。Runtime 则负责最后的执行和校验，因为模型生成了 tool call，不代表这个 tool call 就应该被执行。

### 3.3 候选集管理：不要把所有工具都交给模型

候选集管理解决的是一个更底层的问题：**当前这一步，模型应该看见哪些工具定义？**
...
```text
第一层：工具索引
只给模型看工具名、简短描述、适用场景、风险等级。

第二层：候选工具定义
系统筛出候选集后，再给模型看候选工具的完整 description、参数 schema、返回字段和使用边界。

第三层：深层说明 / 示例 / 脚本
只有当模型真的需要使用某个复杂工具时，才加载详细文档、调用示例、领域规则或辅助脚本。
```
