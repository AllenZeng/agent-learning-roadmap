...
```python
process_tool_result(result, max_chars=2000):
    if result is error:
        return result                    # 错误信息通常较短，直接返回
    content = result.data.content
    if len(content) <= max_chars:
        return result                    # 策略1：内容短 → 直接返回
    else:
        return {
            status: "success",
            summary: result.summary,
            preview: head(content, 500) + "..." + tail(content, 500),  # 策略2：截断
            full_ref: result.data.path,   # 完整内容写入临时文件，给引用路径
...
```python
build_observation(tool_name, raw_result, task_state):
    obs = { tool, status, summary }
    if success:
        obs.data_summary = raw_result.data      # 摘要，非原始数据
        obs.context_hint = generate_success_hint(...)
    if error:
        obs.error = { code, retryable, suggested_action }
        obs.context_hint = generate_error_hint(...)
        # 根据错误码给出针对性提示：
        #   file_not_found → "可用 list_files 查看当前目录"
        #   permission_denied → "不要重复尝试，请求用户授权"
        #   timeout → "可重试 N 次，连续超时应停止"
    return obs

# 完整流程
agent_loop_step(state, llm, tools):
    decision = llm.decide(state.context)        # 1. 模型决策
    raw_result = execute_tool(...)              # 2. Runtime 执行
    observation = build_observation(...)         # 3. 构建 Observation（关键）
    state.add_observation(observation)           # 4. 写入状态，进入下一轮上下文
```
...
```python
# 以 read_file("large_report.md") 为例展示两者的分工

# State Update：完整档案（供调试、审计、恢复）
state.record_tool_call(
    tool="read_file", args={"path": "large_report.md"},
    raw_result=raw_result, duration_ms=1234,
)

# Observation：决策简报（进入下一轮上下文，经过筛选和摘要）
observation = {
    status: "success",
    summary: "成功读取 large_report.md，共 15000 字符",
    preview: content[:800] + "...",
    full_ref: "state://tool_results/step_3/raw",   # 模型需要时可获取完整内容
    context_hint: "报告含 Q1-Q4 销售数据。第 3-5 章为关键分析章节。",
}
...
```python
AuditEntry 记录字段：
    timestamp, user_id, task_id, tool_name, arguments
    permission_result   # "allowed" | "denied" | "confirmed_by_user"
    confirmed_by        # 用户确认时记录用户 ID
