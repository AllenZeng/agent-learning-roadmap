...
- [第四章：最小 ReAct Agent 的实现思路](#第四章最小-react-agent-的实现思路)
  - [4.1 为什么第一次实现不建议用框架](#41-为什么第一次实现不建议用框架)
  - [4.2 伪代码结构](#42-伪代码结构)
  - [4.3 工具设计](#43-工具设计)
  - [4.4 Trace 记录](#44-trace-记录)
  - [4.5 基础错误处理](#45-基础错误处理)
  - [4.6 测试任务设计](#46-测试任务设计)
- [课后练习](#课后练习)
- [验收标准](#验收标准)
- [参考资料](#参考资料)
...
这个机制造就了它惊人的能力——它能理解复杂指令、写出优美的文章、在数学题上一步步推导。但它也划定了模型调用的天然边界：**模型本身不会执行外部动作。** 它可以生成文本，也可以生成结构化的工具调用意图，但真正读取文件、查询数据库、运行代码、写入结果的，仍然是模型之外的 Runtime 和工具层。模型在一次调用中能够依据的，也只是本次请求传入的上下文。
...
> 读到这你可能会问——**Runtime 到底是什么？** 前面反复提到它：状态要它管、工具要它执行、循环要靠它控制。你可以把 Runtime 理解为 Agent 的**操作系统**。就像操作系统管理 CPU、内存、磁盘、进程调度——但它不帮你写文档、做表格；Runtime 管理状态存储、工具执行、循环控制、权限校验、Trace 记录——但它不做语义推理、不生成文本。<br/>
...
**Prompt 不是你随手写给 LLM 的一段备注——它是你用自然语言写的"程序"。** 和其他程序一样，它需要设计、需要测试、需要迭代。
...
当前**在第一次学习最小 agent 闭环时，先让变量可控**——用一个明确的目标，你就能把注意力集中在观察链路本身是否正常运转，而不是纠结"Agent 的行为到底是目标太模糊导致的，还是链路有 bug 导致的"。

另一方面，User Goal 在闭环中的角色是**锚点**——后续每一步决策都以它为参照：当前的动作是否在朝着目标推进？当前的结果是否离目标更近了？最终答案是否覆盖了目标的所有要求？如果这个锚点是模糊的，整个循环就会在不确定中漂移。
...
## 第四章：最小 ReAct Agent 的实现思路

前三章讲完了"是什么"和"为什么"。这一章讲"怎么做"——给出一个最小 ReAct Agent 的实现方案，不绑定任何特定语言或框架。
...
下面是一个最小 Agent Loop 的 Python 骨架。它不是完整实现，只保留闭环的核心结构：
...
- **Context Assembly**：每一轮都从 `state` 中取信息，重新组装模型上下文。
- **LLM Decision**：模型只提出下一步，例如调用工具、给最终答案、请求用户补充或失败退出。
- **Interaction / Execution**：工具由 Runtime 校验和执行，不由模型直接执行。
- **State Update**：每轮把决策和 Observation 写回 `state`，供下一轮继续使用。
- **Stop Check**：最大步数、重复动作、连续错误等硬约束由 Runtime 判断。

`assemble_context()`、`execute_tool()` 和 `check_stop()` 在这里故意写成占位函数，因为本节要展示的是闭环结构。工具参数校验、错误包装、重复动作检测、Trace 记录等细节，会在后面的工具设计、Trace 和错误处理小节展开。
...
按照第四章的任务类型，设计你的 10 条测试任务。每条任务写清楚：用户输入、期望工具调用链、成功标准、可能失败点和为什么它会失败。
...
- [ ] 我能用一句话说清最小 Agent 的五个组成：Prompt、LLM 决策、工具/环境交互、State 管理和循环控制。
- [ ] 我能画出最小运行链路，并标注 Context Assembly、Observation、State Update 和 Continue or Stop 的位置。
- [ ] 我能解释"模型决策，Runtime 执行"的分工，并举例说明 Runtime 至少需要校验什么。
- [ ] 我能区分 Prompt、Context Assembly、State 和长期 Memory 各自负责什么。
- [ ] 我能为一个"读取文件并生成摘要"的任务设计 Prompt、State、3 个工具和基础停止规则。
- [ ] 我能设计 10 条测试任务，并为每条写出成功标准和可能失败点。
