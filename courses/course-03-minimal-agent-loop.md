...
这个机制造就了它惊人的能力——它能理解复杂指令、写出优美的文章、在数学题上一步步推导。但它也划定了裸模型调用的天然边界：**模型本身不会执行外部动作。** 它可以生成文本，也可以生成结构化的工具调用意图，但真正读取文件、查询数据库、运行代码、写入结果的，仍然是模型之外的 Runtime 和工具层。模型在一次调用中能够依据的，也只是本次请求传入的上下文。
...
LLM 的"记忆"完全依赖本次请求中的上下文。它本身没有由应用自动继承的持久化任务状态——如果 Runtime 不把目标、历史、工具结果重新注入上下文，下一次模型调用就无法可靠知道上一次发生了什么。
...
ReAct 论文的作者做了一个关键实验来证明这一点：同一个 frozen PaLM-540B 模型，**普通问答式提示和 ReAct 风格的 few-shot 轨迹提示，表现天差地别**。不是模型变强了——是 Prompt 告诉了模型它应该怎么表现。
...
回到 ReAct 论文。论文的核心贡献不是一个新模型架构，不是一个新训练方法，而是一个 **Prompt 设计**。作者用 frozen PaLM-540B（参数完全冻结，不做任何微调），通过精心设计的 few-shot 轨迹提示，让模型展现出了"推理—行动—观察"的 Agent 行为。

```text
ReAct 的本质：Prompt Engineering 方法
  输入：用户问题 + 精心设计的 ReAct 风格 Prompt（含少量 few-shot 轨迹示例）
...
图里的 `State Store` 不在主链路上，而在旁边。这是有意设计的。
...
- **工具结果变成结构化 Observation**：无论成功还是失败，返回到状态中的 Observation 至少要包含 `status` 和 `summary`，并按情况携带 `content` 或 `error`。在正式实现中，也可以固定四个字段都存在，用 `null` 表示不适用。
