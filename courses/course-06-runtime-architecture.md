...
- 有时模型又开始提变量命名建议。trace 里确实有"只检查 bug"这条规则，但它被长工具结果和历史讨论淹没了。
- 有时模型引用了历史 PR 的旧结论。RAG 服务返回的片段本身没有错，错的是阶段①没有处理"过期信息能不能进入本轮上下文"。
- 有时成本突然翻倍。原因是某个工具结果从 300 行变成 5000 行，但阶段①没有预算、压缩和告警。
- 有时你改了压缩策略，代码审查质量变差。问题不是"压缩这个策略不对"，而是没人能说清楚当时是哪一层信息被压掉、为什么被压掉、用的是哪个策略版本。

这时团队会很容易回到课程五的讨论："要不要调预算？要不要换 rerank？要不要把约束放前面？"这些讨论没有错，但它们还停留在**上下文策略**层面。

课程六要追问的是另一个问题：这些策略在 Harness 主循环里由谁执行？谁保证每一轮进入模型的内容都经过同一套规则？谁在策略变化后留下可回放的证据？谁在关键约束缺失时阻止阶段②继续调用模型？
...
本章的核心不是再讲一遍"上下文应该怎么处理"（课程五已经讲了），而是把这些处理策略落到 Harness 的阶段①里。**Context Pipeline 要成为主循环阶段①的一条可被调用、可被测试、可被观测、可被替换的运行时管线。**
