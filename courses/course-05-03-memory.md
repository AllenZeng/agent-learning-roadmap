...
不是所有对话都值得记住。回到 3.3.2 的会话片段，系统需要在对话过程中持续识别候选记忆。以下是识别逻辑的具体实现——**注意这不是抽象的"应该识别"，而是可运行的判别代码**：
...
注意这里识别出了 3 条候选——一句话里包含了三个独立的偏好信号。"写文章先大纲"和"语气直接"和"不要营销化"虽然是同一句话，但在存储时应该合并为一条关联记忆，而不是三条碎片。这个合并逻辑在下一节的写入决策中处理。
...
def should_remember(candidate: dict, context: dict) -> tuple[bool, str]:
    ...
    if candidate.get("source") == "inferred" and conf < 0.7:
        return False, f"推断置信度不足 ({conf:.2f} < 0.7)，保持为候选"
...
最终只有 1 条记忆被写入（3 个信号合并为 1 条复合偏好），不是 3 条碎片。
...
# │   影响：Agent 可能用 TS 示例（但这条是推断的，置信度不高）          │
# ├──────────────────────────────────────────────────────────────────┤
...
- **置信度传递**：推断的记忆（如"可能偏好 TS"）在上下文中应该附带置信度标注，让模型知道这是推测而非确定偏好。
...
│  → identify_candidates: 3 条候选（先大纲、语气直接、不营销）
│  → should_remember: 3 条合并为 1 条复合偏好
│  → write: ✅ 写入 preference（id: mem_a1b2c3）
│
...
│  → write: ⚠️ 写入候选记忆（needs_confirmation: true），待用户确认
│
...
│  ┌────────────────────────────────────────────────────────────┐
│  │ preferences:                                               │
│  │   mem_a1b2c3: "写文章先大纲、语气直接不营销" (active)       │
│  │ facts:                                                     │
...
│  → 结果：
│    #1 (0.91): "写文章先大纲、语气直接不营销" ← 跨会话仍然有效！
│    #2 (0.72): "先大纲再正文在技术文章中效果好"
│    #3 (0.55): "用户可能偏好 TS" ← 置信度低，附带标注
...
│  │ facts:                                                     │
│  │   mem_d4e5f6: "用户可能偏好 TS" (pending, access_count: 1) │
│  │ task_history:                                              │
...
4. **推断 vs 确认**："可能偏好 TS"是推断的（confidence 0.5），"示例代码用 Python"是用户明确的（confidence 0.95）——后者优先级更高。
...
该示例用纯 Python 标准库演示知识助手 Memory 系统的完整生命周期：会话管理、候选记忆识别、写入决策守卫、分层存储（偏好/事实/任务历史）、语义召回、冲突检测与替换、衰减清理、审计日志。示例包括一个交互式 REPL，你可以在其中模拟周一→周二→周三的跨会话对话，观察 Memory 系统如何记住、更新和遗忘。
