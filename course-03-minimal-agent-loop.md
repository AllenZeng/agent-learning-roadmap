...
```text
Agent = LLM 决策 + 工具/环境交互 + State（状态管理） + 循环控制
```
...
4. **理解 State（状态）管理的角色** — 知道哪些信息需要独立于 LLM 存储，以及为什么上下文窗口不能作为唯一的 State 存储
...
- [第三章：最小运行链路](#第三章最小运行链路)
  - [3.1 最小闭环图](#31-最小闭环图)
  - [3.2 环节总览：一条链路串起四个组成部分](#32-环节总览一条链路串起四个组成部分)
  - [3.3 User Goal：从模糊意图到可操作目标](#33-user-goal从模糊意图到可操作目标)
  - [3.4 Context Assembly：模型应该看到什么](#34-context-assembly模型应该看到什么)
  - [3.5 LLM Decision：下一步做什么](#35-llm-decision下一步做什么)
  - [3.6 Interaction & Execution：真的去做](#36-interaction--execution真的去做)
  - [3.7 Observation / Feedback：让结果回到循环](#37-observation--feedback让结果回到循环)
  - [3.8 State Update：把本轮发生的事写进 State](#38-state-update把本轮发生的事写进-state)
  - [3.9 Continue or Stop：知道什么时候该停](#39-continue-or-stop知道什么时候该停)
  - [3.10 State 存储的正确位置](#310-state-存储的正确位置)
  - [3.11 核心模块与连接点的区别](#311-核心模块与连接点的区别)
  - [3.12 最小闭环背后的工程原则](#312-最小闭环背后的工程原则)
...
第二章讲了四个组成部分分别是什么——LLM 决策、工具/环境交互、State 管理、循环控制。这一章回答它们怎么串成一条闭环。

我们换个讲法。先给你一张全景图——把整条链路画出来，标注 Runtime 的边界、State 的位置、数据流动的方向。你对"每个环节在哪、谁管谁"建立起坐标系之后，再逐个环节深入。
...
### 3.2 环节总览：一条链路串起四个组成部分

有了全景图，下面把链路按文字展开：

```text
User Goal
    → Context Assembly（连接点：State → LLM 的桥梁）
...
七个环节中，四个是核心模块（LLM 决策、工具交互、State 管理、循环控制），两个是运行时连接点（Context Assembly、Observation / Feedback），一个是用户输入锚点（User Goal）。3.11 节会详细解释"核心模块"和"连接点"的区分。

下面逐个环节展开。

### 3.3 User Goal：从模糊意图到可操作目标
...
### 3.6 Interaction & Execution：真的去做
...
### 3.7 Observation / Feedback：让结果回到循环
...
### 3.8 State Update：把本轮发生的事写进 State
...
以上七个环节走完，你对闭环的"数据流动"已经有了完整的认知。下面三节（3.10-3.12）不再引入新环节，而是对闭环中三个最容易被误解的点做更深的分析：State 为什么放在图旁边而不是链路中？Context Assembly 和 Observation 为什么不算核心模块？整条链路背后统一的工程原则是什么？
...
Context Assembly 是 State 管理和 LLM 决策之间的连接点：

```text
State 里有什么 → 选择什么进入上下文 → 模型据此决策
```
