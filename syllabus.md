...
| 维度 | 观察问题 |
|---|---|
| 任务入口 | 用户输入的是目标、指令还是问答？ |
| 下一步决策权 | 由用户、固定流程还是模型决定？ |
| 行动能力 | 能调用哪些外部能力？ |
| 过程可见性 | 用户能看到哪些步骤？ |
| 用户控制权 | 能否确认、中断、修改或接管？ |
| 失败与恢复 | 出错后如何处理？ |
...
运行链路：`User Goal → Context Assembly → LLM Decision → Tool/Environment Interaction → Observation/Feedback → State Update → Continue or Stop`
...
工程原则：模型负责理解目标和判断下一步，确定性基础设施负责工具执行、权限、安全、状态、恢复和观测。最小闭环必须有任务状态和循环控制，但不一定需要长期 Memory 或 RAG。
...
设计原则：Deny-first（默认拒绝）、渐进式授权、工具安全是子系统而非几个 if 判断。
---
...
判断框架：当前场景问题 → 哪个能力能解决 → 增加什么复杂度 → 为什么值得/不值得引入。不需要引入的典型信号同样重要。
---
...
核心思路：把课程三的裸 ReAct 循环工程化为可追踪、可恢复、可评测、可治理的 Harness。
---
