# 课程四：Agent 架构深入 —— 从能用到好用

## 课程导言

课程三让你实现了一个能跑起来的 ReAct Agent，但"能跑"和"好用"之间还有巨大的鸿沟——你会遇到死循环、上下文溢出、无法衡量好坏、越界操作、黑盒调试等问题。课程四要回答的核心问题是：**如何用工程化的架构设计，系统性地解决这些问题？**

本课程从 Harness（运行时引擎）出发，建立驱动层/控制层/管理层的三层架构认知。随后深入 Context Engineering（上下文工程）——当 Prompt、RAG、Memory、Tool Use 都在往同一个上下文窗口里塞东西时，如何管理信息密度和位置优先级？接着覆盖 Skill/Plugin 的能力封装、Orchestration（编排调度）的四种模式、Memory 的工程化架构、Evaluation（评测）的方法论、Guardrails（安全护栏）的设计、以及 Observability（可观测性）的 Tracing 体系。

每个主题都遵循"背景与痛点 → 核心思想 → 方案详解 → 架构对比 → 未来展望"的讲述逻辑，帮助你知其然也知其所以然。

> **与课程三的衔接**：课程三的"文件整理 Agent 桥梁项目"在复盘时会发现 5 类问题（死循环 / 遗忘 / 无法衡量好坏 / 越界操作 / 黑盒调试）。本课程中，Harness（第一章）是所有这些问题的**统一入口**——它把驱动、控制、管理三层解耦，让你可以单独优化每一层而不牵动全局；Context Engineering 则回答一个贯穿性的问题——每次 LLM 调用时，上下文窗口里塞的这些东西到底值不值；编排、记忆、评测、护栏、观测分别解决 5 类具体问题。建议在学习每个主题时，回看你的复盘报告和课程三的"裸循环"代码，同时关注每个主题对上下文窗口的影响。
>
> **重要提示**：本课程的 Evaluation（第六章）内容，实际上从课程三你就应该开始实践——哪怕只是 10 条测试任务的简单评测集。本课程会系统化地讲 Evaluation 的理论和方法，但**不要等到学完再开始评测**。评测是贯穿始终的实践，不是学完理论才做的事。
...
2. 运用 Context Engineering 原则管理上下文窗口：信息密度优先、位置即优先级、分层管理、性价比思维
3. 根据任务复杂度选择合适的编排模式（Chain / Router / Plan-Execute / ReAct Loop / Graph）
4. 为 Agent 设计分层记忆系统（短期 / 工作 / 长期），并知道每层的适用场景
5. 从零搭建 Agent 评测框架，包含端到端评测和步骤级评测
6. 实现输入/输出 Guardrails，设计合理的 Human-in-the-Loop 确认策略
7. 为 Agent 集成 Tracing，能快速定位失败原因

---

## 目录

- [第一章：Agent Harness —— 运行时引擎](#第一章agent-harness--运行时引擎)
- [第二章：Context Engineering —— 上下文窗口的工程化](#第二章context-engineering--上下文窗口的工程化)
- [第三章：Skill 与 Plugin —— 能力封装与复用](#第三章skill-与-plugin--能力封装与复用)
- [第四章：Orchestration —— Agent 的调度大脑](#第四章orchestration--agent-的调度大脑)
- [第五章：Memory 架构深入 —— 从"能记住"到"记得聪明"](#第五章memory-架构深入--从能记住到记得聪明)
- [第六章：Evaluation —— 如何衡量 Agent 的好坏](#第六章evaluation--如何衡量-agent-的好坏)
- [第七章：Guardrails —— 让 Agent 在安全边界内运行](#第七章guardrails--让-agent-在安全边界内运行)
- [第八章：Observability —— 看清楚 Agent 在做什么](#第八章observability--看清楚-agent-在做什么)
- [课程总结](#课程总结)
- [补充专题：MCP 协议 —— 工具标准化的工程实践](#补充专题mcp-协议--工具标准化的工程实践)
- [练习任务](#练习任务)
- [验收标准](#验收标准)

---

## 第一章：Agent Harness —— 运行时引擎

在正式进入编排、记忆、评测等高级主题之前，我们需要先建立一个基础概念：**无论你的 Agent 采用什么编排模式，它都需要一个"运行时"来驱动——这就是 Harness。**

课程三的桥梁项目中，你已经实现过 Harness——那个 `for step in range(max_steps)` 循环。本课的目标是：**把这个"裸循环"提升为有意识的架构设计**。

### 0.1 什么是 Harness

Harness 是 Agent 的**运行时引擎**。它不参与具体的推理（那是 LLM 的事），也不执行具体的工具（那是 Tool 的事），它只做一件事：**驱动 Agent 循环，直到任务完成或被终止**。

> **术语来源**："Agent Harness"作为 Agent 工程的基础概念，在 2025-2026 年间逐步形成行业共识。LangChain 工程博客将其定义为"模型中不包含的所有代码、配置和执行逻辑"；Cloudflare Agents 文档将其描述为"让 Agent 像一个 Agent 而不是单次模型调用的循环"；学术论文《AI Harness Engineering: A Runtime Substrate for Foundation-Model Software Agents》（arXiv, 2026）系统化地提出了 Harness 的组件职责和 H0-H3 成熟度模型。
...
## 第二章：Context Engineering —— 上下文窗口的工程化

### 为什么需要 Context Engineering

课程三中，我们分别学习了五大核心模块——每个模块解决 LLM 的一个根本局限。但当你把这些模块组合成一个真实的 Agent 时，一个元问题浮现了：**它们都在往同一个上下文窗口里塞东西**。

- Prompt Engineering 塞入 System Prompt 和 Few-shot 示例
- RAG 塞入检索到的文档片段
- Memory 塞入历史对话摘要和用户偏好
- Tool Use 塞入工具描述和执行结果
- 每个 Thought-Action-Observation 循环都往窗口里追加新内容

而上下文窗口是有限的（即使 200K tokens 也不够挥霍），且 LLM 对不同位置的信息**关注度不均匀**（Lost in the Middle 现象）。Context Engineering 要回答的核心问题就是：**这扇窗口里应该放什么、按什么顺序放、什么该扔、什么该压。**

如果说 Prompt Engineering 是写好指令，Context Engineering 就是管好模型的"视野"——Prompt 只是视野里的一小块，整个视野的质量决定了 Agent 决策的质量。

### 上下文窗口的组成

一次典型的 Agent LLM 调用，上下文窗口由以下几层组成：
...
│  ┌────────────────────────────────────┐      │
│  │  ④ 对话历史（滚动层）               │      │
│  │  User: 整理 /downloads 目录         │      │
│  │  Assistant: Thought → Action → ... │      │
│  │  最近 N 轮 Thought-Action-Observation│     │
│  └────────────────────────────────────┘      │
│  ┌────────────────────────────────────┐      │
│  │  ⑤ RAG 检索结果（按需注入层）       │      │
│  │  "相关文档1：文件命名规范.pdf..."    │      │
...
│  ┌────────────────────────────────────┐      │
│  │  ⑥ 当前任务（用户输入层）            │      │
│  │  "把昨天下载的 PDF 移到 documents"   │      │
│  └────────────────────────────────────┘      │
└──────────────────────────────────────────────┘
```

每一层都在争夺窗口里的 token 配额。一个糟糕的 Context Engineering 会让 70% 的窗口塞满重复或无关内容，而 Agent 真正需要的信息被挤到了 LLM 注意力最弱的"中间地带"。

### 四个核心原则

**原则一：信息密度优先（Density over Volume）**

每塞入一个 token，问自己：这个 token 增加了 LLM 做出正确决策的概率吗？如果不是，删掉。

典型反模式：
- 工具描述写了 500 字，但 LLM 只需要知道"这个工具是干什么的、参数怎么填"
- RAG 返回了 10 个文档片段，但只有前 2 个相关，后 8 个在分散 LLM 的注意力
- 对话历史保留了 20 轮，但只有最近 5 轮对当前任务有参考价值

提升信息密度的常用手段：
- **工具描述精炼**：每个工具用一句话描述核心功能 + 一句话说明何时使用 + 参数列表。不要写教科书式的文档。
- **检索结果压缩**：对 RAG 返回的片段做摘要后再注入，或只保留最相关的 Top-3。
- **历史对话摘要**：超过 N 轮后，不直接截断（会丢失关键信息），而是让 LLM 生成摘要替代原始对话。

**原则二：位置即优先级（Position is Priority）**

LLM 对上下文窗口不同位置的关注度存在显著差异。大量研究证实：窗口开头（primacy effect）和结尾（recency effect）的信息被重点关注，中间位置（Lost in the Middle）关注度最低。

因此，上下文窗口的"地产"是有价值的——窗口开头和结尾是黄金地段：
...
上下文窗口关注度分布（概念示意）：
  ...
  低 │ ████████          ░░░░░░         ████████
     └──────────────────────────────────────────
      开头（~10%）      中间（~80%）     结尾（~10%）
     System Prompt +    检索结果 +     最新对话 +
     工具定义 + 角色     历史中间段      当前任务
```

推断：
- **最重要、最不变的信息放开头**：System Prompt、工具定义、角色规范。它们始终在 LLM 的"眼皮底下"。
- **当前任务和最新观察放结尾**：最新一轮的 Thought-Action-Observation 是 LLM 下一步决策的直接依据，必须放在注意力最强的地方。
- **检索结果和记忆注入夹在中间**：这是最危险的位置。如果检索到的文档很长，关键信息可能被"淹没"。解决方案：精简检索结果，或使用 Rerank 确保最相关的片段靠前。

**原则三：分层管理（Layered Management）**

不把所有内容混在一个扁平列表里，而是按稳定性和作用分层：

| 层级 | 内容 | 更新频率 | 管理策略 |
|------|------|---------|---------|
| **固定层** | System Prompt、工具定义 | 几乎不变 | 设计时优化，运行时不变 |
| **会话层** | 对话历史、工具调用结果 | 每轮追加 | 滑动窗口 + 摘要压缩 |
| **按需层** | RAG 检索结果、记忆查询结果 | 需要时注入 | 仅在相关时出现，用后即弃 |
| **用户层** | 当前任务、用户指令 | 每次不同 | 不可修改，不可压缩 |

分层管理的核心价值：当你需要优化上下文时，你知道从哪里下手——会话层的对话历史太长了就压缩它，按需层的检索结果太多了就精简它。固定层和用户层不参与动态调整。

**原则四：性价比思维（Token Economics）**

每多塞 1000 tokens，不只是多花一点钱——延迟增加、Attention 稀释、留给后续步骤的窗口空间减少。上下文窗口是一个"公共资源"，每一层都在消费它，但没有一层为"过度消费"负责。

实用准则：
- 问自己：如果只能给 LLM 500 tokens 的上下文（除去固定层），你会放什么？这个答案就是你的上下文 MVP。
- 在每次 LLM 调用后，检查实际消耗的 tokens vs 你认为"有用"的 tokens。如果比例低于 50%，说明上下文里有大量冗余。
- 工具描述和 System Prompt 是"沉默的消费者"——它们每次都消耗 tokens，但很少被审视。定期审查：这个工具描述里的每一句话，LLM 真的需要吗？

### Context Engineering 在 Agent 循环中的位置

将 Context Engineering 放入 Harness 的视角中，每次 LLM 调用前都经历一个"上下文组装"步骤：
...
│                  └──────────┘                       │
│                                                     │
│   "上下文组装"就是 Context Engineering 的主场：       │
│   从固定层/会话层/按需层/用户层中，                     │
│   按优先级和密度原则，拼出这一轮的最优窗口              │
└─────────────────────────────────────────────────────┘
```

这就是 Context Engineering 和 Harness 的关系：Harness 管循环，Context Engineering 管每次循环 LLM "看到什么"。两者配合，才是完整的运行时。

### 与其他核心模块的关系

| 相关模块 | 关系 |
|---------|------|
| **Prompt Engineering** | Prompt 是 Context 的一个子集。写好 Prompt 是基础，但 Prompt 写得再好，如果被 RAG 垃圾结果和 50 轮历史淹没，也没用。Context Engineering 是 Prompt Engineering 的上层框架。 |
| **RAG** | RAG 负责"找到相关文档"，Context Engineering 负责"把找到的文档以正确的量、正确的位置、正确的格式放进窗口"。检索质量 × 注入质量 = LLM 实际利用的知识。 |
| **Memory** | Memory 负责"记住什么"，Context Engineering 负责"在正确的时机把正确的记忆放进窗口"。一条有价值的记忆，如果总是在 LLM 注意力最弱的时候注入，等于白记。 |
| **Orchestration** | Orchestration 决定"下一步做什么"，Context Engineering 确保"做决策时看到了该看的东西"。错误的编排会导致错误的方向，错误的上下文会导致"看着正确的方向却走错了路"。 |
| **Harness** | Harness 是运行时引擎，Context Engineering 是引擎的"燃料系统"——每次点火前，确保燃料（上下文）的配比正确。 |

### 一句话总结

**Prompt Engineering 告诉你"怎么对 LLM 说"，Context Engineering 告诉你"让 LLM 看见什么"。后者决定前者的天花板——指令再清晰，如果 LLM 的视野里堆满了噪音，它也做不出好决策。**

---

## 第三章：Skill 与 Plugin —— 能力封装与复用

Tool Use 赋予了 Agent 调用外部能力的基础，但单个 Tool 的粒度太细——一个完整的"邮件处理"能力可能涉及读邮件工具、发邮件工具、日历工具、摘要提示词、联系人知识等。**Skill（技能）和 Plugin（插件）是将多个原子能力打包为可复用能力单元的高层抽象**。

### 为什么需要 Skill / Plugin

在真实 Agent 应用中，你会反复遇到三个问题：

1. **能力复用**：同样的"代码审查"流程，在 Code Review Agent 和 CI/CD Agent 中都要用，但每次都要重新组合工具+提示词
2. **组合复杂**：一个"数据分析"能力 = SQL 查询工具 + 可视化工具 + 统计领域知识 + 分析模板提示词，这不是单个 Tool 能表达的
3. **安全边界**：你希望第三方贡献"天气查询"能力，但不能让它访问你的文件系统——需要能力级别的沙箱

这三个问题的共同答案就是 Skill / Plugin。

### Skill vs Plugin vs Tool：三者的层次关系
...
│  │  描述：我是什么、何时触发、怎么做      │    │
│  │  ┌──────────┬──────────┬─────────┐  │    │
│  │  │ Tool A   │ Tool B   │ Tool C  │  │    │  ← 原子工具
│  │  ├──────────┼──────────┼─────────┤  │    │
│  │  │ Prompt   │ Knowledge│ State   │  │    │  ← 提示词 + 知识 + 状态
│  │  └──────────┴──────────┴─────────┘  │    │
│  └─────────────────────────────────────┘    │
│  + 触发条件 + 权限声明 + 版本号              │
└─────────────────────────────────────────────┘
...
| 层次 | 粒度 | 示例 | 特征 |
|------|------|------|------|
| **Tool** | 原子操作 | `search_database(sql)`、`send_email(to, body)` | 单一函数，有 Schema 定义 |
| **Skill** | 能力组合 | "邮件摘要" = 读邮件 + 摘要提示词 + 发送草稿 | 工具集 + 提示词 + 知识，通常由开发者定义 |
| **Plugin** | 可分发的 Skill | "Slack 插件" = 发消息 + 读频道 + 搜索历史 | Skill + 版本管理 + 安全声明 + 分发机制 |

**核心区分**：Tool 回答"能做什么操作"；Skill 回答"能完成什么任务"；Plugin 回答"如何安全地获得一个 Skill"。

在实际生态中，这两个术语经常混用：ChatGPT 叫 GPTs/Actions，Claude Code 叫 Skills/Commands，LangChain 叫 Tools（但粒度已接近 Skill）。**本文用 Skill 指代能力封装单元，用 Plugin 强调分发和安装的工程维度**。

### Skill 的核心设计要素

一个设计良好的 Skill 包含以下组件：

| 组件 | 说明 | 示例（"代码审查" Skill） |
|------|------|------------------------|
| **声明式描述** | Skill 的名字、一句话说明、详细描述，帮助 Router Agent 决定何时激活 | "审查代码变更，检查安全漏洞、性能问题和代码风格" |
| **触发条件** | 什么场景下应该激活此 Skill？关键词 / 意图分类 / 显式调用 | 用户说 "review this PR" 或检测到 git diff |
| **工具集** | 此 Skill 可以调用的所有 Tool | `git_diff()`、`run_linter()`、`run_tests()` |
| **上下文注入** | 激活此 Skill 时自动注入的 System Prompt 片段 | "你是资深代码审查员。关注：SQL 注入、空指针、死锁…" |
| **知识库** | Skill 专属的 RAG 知识源 | 公司编码规范文档、CWE Top 25 知识库 |
| **输出 Schema** | Skill 返回的结构化结果格式 | `{ "issues": [...], "severity": "high/medium/low", "summary": "..." }` |

### 动态加载与 Skill 路由

Skill 的核心工程挑战在于**动态发现和路由**：

```
用户输入："帮我分析上周的销售数据"
     │
     v
Router Agent（Skill 选择器）
     │
     ├── 匹配 Skill "数据分析"（关键词：分析、数据）
     │     └─ 激活 → 注入 SQL 查询 Tool + 统计知识 + 可视化 Prompt
     │
     ├── 匹配 Skill "日程管理"（关键词：上周）
     │     └─ 不激活（意图不匹配）
     │
     └── 不匹配 Skill "代码审查"
           └─ 不激活
...
- **显式调用 vs 自动检测**：用户说 "review this code" → 是显式调用 Skill？还是自动检测意图？前者可靠但要求用户知道有哪些 Skill，后者智能但可能误判。实际系统中通常两者结合：Router 自动匹配，用户也可以显式 `@skill-name` 调用
- **单 Skill vs 多 Skill 并行**：复杂任务可能需要多个 Skill 协作。例如"帮我审查代码并发邮件给团队"涉及 Code Review Skill + Email Skill
- **Skill 冲突处理**：当两个 Skill 的触发条件重叠时，按优先级 / 置信度 / 显式声明解决

### 与 MCP 的关系

MCP 协议的 Tools / Resources / Prompts 三元组本质上为 Skill 提供了**标准化定义格式**：

| MCP 能力 | Skill 组件 | 关系 |
|----------|-----------|------|
| `tools/list` | 工具集 | MCP Server 暴露的工具 → Skill 声明使用哪些 |
| `resources/list` | 知识库 | MCP 提供知识源 → Skill 声明引用哪些 |
| `prompts/list` | 上下文注入 | MCP 提供提示词模板 → Skill 声明激活时注入哪些 |

换句话说，**MCP 解决了"能力如何被对方看到和调用"，Skill 解决了"能力如何被有组织地打包和复用"**。一个 MCP Server 可以暴露多个 Skill，一个 Skill 也可以跨多个 MCP Server 组合工具。

### Skill 生态示例

| 平台/框架 | Skill 形态 | 特点 |
|----------|-----------|------|
| **Claude Code** | `.claude/skills/` 目录 + Markdown 格式 | 声明式，文件即 Skill，自动发现 |
| **ChatGPT** | GPTs + Actions | 可视化配置 + API 绑定，面向终端用户 |
| **LangChain** | `@tool` → `StructuredTool` | 代码优先，可组合为 Chain/Agent |
| **MCP Marketplace** | MCP Server 列表 | 标准协议，跨框架可移植 |
| **Copilot Extensions** | GitHub App + Agent | 事件驱动，深度集成开发流程 |

### 实践建议

1. **从 Tool 开始，不急于抽象 Skill**：当你发现同一个 Tool 组合被使用了 3 次以上，再抽象为 Skill
2. **Skill 的描述比实现更重要**：Router Agent 能否选对 Skill，80% 取决于 Skill 描述写得好不好
3. **给每个 Skill 定义明确的"不适用范围"**：代码审查 Skill 的描述里写 "不适用于配置文件检查" 比事后拦截更高效
4. **Skill 需要独立测试**：每个 Skill 应有自己的评估用例，验证"触发→工具调用→结果"的完整链路
5. **版本管理不可省略**：Skill 的 Prompt 或工具集变更可能导致行为退化，应有版本号和 changelog

### 与后续课程的关系

Skill / Plugin 的概念贯穿后续几门课程：

| 后续课程 | Skill 相关话题 |
|---------|---------------|
| **第八章：Observability** | 需要追踪 Skill 激活/切换事件，计算 Skill 级别的成功率 |
| **第七章：Guardrails** | Skill 级别的权限控制——"邮件 Skill 不能访问 Git 仓库" |
| **课程五（生产环境）** | Skill 的 CI/CD——如何安全地热更新线上 Skill |
| **课程七（发展趋势）** | Skill 生态的标准化——Skill 可发现性、Marketplace 模式 |

### 一句话总结

**Tool 是动词（做什么操作），Skill 是句子（完成什么任务），Plugin 是短语书（如何获得和管理一组句子）。把反复出现的工具+提示词+知识组合封装为 Skill，是 Agent 从 Demo 走向产品的必经之路。**

---

## 第四章：Orchestration —— Agent 的调度大脑
...
[步骤2]  输出A 作为输入 →  LLM调用 → 输出B
                   
[步骤3]  输出B 作为输入 →  LLM调用 → 最终结果
```
...
## 第五章：Memory 架构深入 —— 从"能记住"到"记得聪明"

> **与课程三的衔接**：课程三的第五课已覆盖了 Memory 的基础概念——上下文窗口管理（滑动窗口、摘要压缩、重要性过滤）、短期 vs 长期记忆的区分、以及一个基于 JSON 文件的最简实现。如果你还没读那一课，建议先回顾。本课在此基础上，深入 Memory 的工程化架构：向量数据库选型、MemGPT 虚拟内存思想、记忆蒸馏、多 Agent 共享记忆等。

### 2.1 记忆的层次结构（回顾与深化）

#### 2.1.1 人类的记忆模型：从认知心理学出发

在课程三中我们提过，人类记忆分为感觉记忆、工作记忆和长期记忆三个层次。Agent 的 Memory 设计直接借鉴了这个模型。本课将更深入地展开这个类比，并介绍工业级的实现方案。
...
## 第六章：Evaluation —— 如何衡量 Agent 的好坏

> **为什么 Evaluation 放这里但应该更早开始？**
> 
> 从课程结构上，Evaluation 放在课程四系统讲解。但从实践上，你应该从课程三的第一个 Agent 就开始写评测集——哪怕只有 10 条任务。Agent 开发最容易陷入的误区是"demo 能跑就以为会了"，但真正的核心能力是稳定性。10 条评测任务就能把"demo 心态"扭转为"系统优化心态"。
> 
> 如果你是从课程三带着评测集过来的，这堂课会让你已有的经验体系化。如果你是先学理论，请学完后立刻回到你的 Agent 项目，为它补上评测集。
...
```
        ┌──────────────┐
        │  端到端评测    │  ← 课程四第六章：任务成功率、LLM-as-Judge
        │  (E2E Eval)   │
       ┌┴──────────────┴┐
...
## 第七章：Guardrails —— 让 Agent 在安全边界内运行
...
## 第八章：Observability —— 看清楚 Agent 在做什么
...
本课程我们从运行时到上下文，再到编排、记忆、评测、护栏、观测，建立了 Agent 架构的完整视角：

| 模块 | 核心问题 | 关键方案 |
|------|---------|---------|
| **Harness** | Agent 如何"跑起来"？ | 三层架构（驱动层 / 控制层 / 管理层），解耦执行与决策 |
| **Context Engineering** | 上下文窗口里该放什么？ | 信息密度优先 / 位置即优先级 / 分层管理 / 性价比思维 |
...
1. **Harness 重构**：将你在课程三写的"裸循环"Agent，重构为三层 Harness 架构（驱动层 / 控制层 / 管理层），对比重构前后的代码可维护性
2. **上下文工程实验**：将你的 Agent 在一次长任务中每轮的上下文窗口内容 dump 出来，统计各层（System Prompt / 工具定义 / 对话历史 / 检索结果 / 记忆注入）的 token 占比。找到占比最高但价值最低的那一层，优化它，对比优化前后的任务成功率
3. **编排模式对比实验**：用同一个任务（如"研究一个技术问题并输出报告"），分别用 Plan-Execute 和 ReAct Loop 两种模式实现，对比效率和结果质量
4. **记忆系统实现**：为你的桥梁项目 Agent 添加工作记忆（用 JSON 文件或 SQLite）和长期记忆（用向量数据库）
5. **评测框架搭建**：为你的 Agent 构建一个至少 20 条任务的评测集，包含正常场景、边界场景和对抗场景
6. **Guardrails 实现**：为工具的输入和输出添加校验逻辑，定义至少 3 个需要 Human-in-the-Loop 的场景
7. **Tracing 集成**：使用 Langfuse 或手动实现，记录 Agent 每次运行的完整 Trace
...
> **参考资料**
>
> - [The Anatomy of an Agent Harness](https://www.langchain.com/blog/the-anatomy-of-an-agent-harness) — LangChain 工程博客，Harness 概念的工程化定义
> - [AI Harness Engineering: A Runtime Substrate for Foundation-Model Software Agents](https://arxiv.org/abs/2605.13357) — 学术论文，Harness 的 11 项组件职责和 H0-H3 成熟度模型
> - [Cloudflare Agents: Harnesses](https://developers.cloudflare.com/agents/harnesses/) — Cloudflare 的 Agent Harness 实现文档
> - [The Agent Loop Is the New OS](https://www.harness.io/blog/agent-loop-new-os) — Harness.io 对 Agent 循环与操作系统类比的论述
