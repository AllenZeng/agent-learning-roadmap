...
> **难度**：中高级 | **前置要求**：课程一、课程三
>
> 本课程从 Context Engineering（上下文工程）出发，深入 Agent 的核心架构模块：Harness（运行时引擎）、Orchestration（编排调度）、Memory（记忆系统）、Evaluation（评测）、Guardrails（安全护栏）、Observability（可观测性）。每个模块都遵循"背景与痛点 → 核心思想与提出过程 → 方案详解 → 架构对比 → 未来展望"的讲述逻辑，帮助大家"知其然也知其所以然"。
...
2. [Context Engineering -- 上下文窗口的工程化](#context-engineering----上下文窗口的工程化)
3. [Skill 与 Plugin -- 能力封装与复用](#skill-与-plugin----能力封装与复用)
4. [第一课：Orchestration -- Agent 的调度大脑](#第一课orchestration----agent-的调度大脑)
5. [第二课：Memory -- Agent 的记忆系统](#第二课memory----agent-的记忆系统)
6. [第三课：Evaluation -- 如何衡量 Agent 的好坏](#第三课evaluation----如何衡量-agent-的好坏)
7. [第四课：Guardrails -- 让 Agent 在安全边界内运行](#第四课guardrails----让-agent-在安全边界内运行)
8. [第五课：Observability -- 看清楚 Agent 在做什么](#第五课observability----看清楚-agent-在做什么)
...
- 已完成课程三（底层核心）的学习（能独立实现 ReAct Agent 和 RAG 系统）
- 已完成桥梁项目（文件整理 Agent）并写了复盘报告
- 手上有一份"踩坑清单"——你在桥梁项目中遇到的所有问题
- 对"为什么需要架构"有自己的困惑和理解

> **重要提示**：本课程的 Evaluation（第三课）内容，实际上从课程三你就应该开始实践——哪怕只是 10 条测试任务的简单评测集。本课程会系统化地讲 Evaluation 的理论和方法，但**不要等到学完再开始评测**。评测是贯穿始终的实践，不是学完理论才做的事。
...
> **本课程与课程三的衔接**：课程三的"文件整理 Agent 桥梁项目"在复盘时会发现 5 类问题（死循环 / 遗忘 / 无法衡量好坏 / 越界操作 / 黑盒调试）。本课程中，Harness（第零课）是所有这些问题的**统一入口**——它把驱动、控制、管理三层解耦，让你可以单独优化每一层而不牵动全局；Context Engineering 则回答一个贯穿性的问题——每次 LLM 调用时，上下文窗口里塞的这些东西到底值不值；编排、记忆、评测、护栏、观测分别解决 5 类具体问题。建议在学习每个主题时，回看你的复盘报告和课程三的"裸循环"代码，同时关注每个主题对上下文窗口的影响。
...
| 后续课程 | Skill 相关话题 |
|---------|---------------|
| **第五课：Observability** | 需要追踪 Skill 激活/切换事件，计算 Skill 级别的成功率 |
| **第四课：Guardrails** | Skill 级别的权限控制——"邮件 Skill 不能访问 Git 仓库" |
