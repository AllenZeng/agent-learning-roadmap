# Agent 学习路线图：从基础认知、到深入核心架构、再到产品化实践

## 课程基本介绍
这份路线图基于个人系统学习 Agent 的过程整理，这是一套从"第一次接触 Agent"到"能设计、实现、评估并上线 Agent 产品"的系统性学习路径，共七个课程。

**学什么**：Agent 不是单一模型能力，而是一套由 LLM、工具调用、知识检索、记忆系统、提示工程、运行时控制、上下文工程、编排调度、评测体系、安全护栏和可观测性共同组成的系统。七个课程从认知建立开始，逐层深入到架构设计、工程实践和产品落地，最后以持续跟进生态收尾。

**怎么学**：每门课程都有明确的目标和实践里程碑——不只是"读过"，而是"做过"。先建立直觉、再理解能力演进、再深入核心、再进入架构、最后回归产品。
...
- [课程一：初识Agent](#课程一初识agent)
- [课程二：Agent 核心能力的演进](#课程二agent-核心能力的演进)
- [课程三：Agent 底层核心](#课程三agent-底层核心)
- [课程四：Agent 架构深入](#课程四agent-架构深入)
- [课程五：Agent 走向生产环境](#课程五agent-走向生产环境)
- [课程六：Agent 项目实战](#课程六agent-项目实战)
- [课程七：Agent 最新发展趋势](#课程七agent-最新发展趋势)
- [课程依赖关系](#课程依赖关系)
- [推荐学习资源](#推荐学习资源)
...
- **核心能力一：Tool Use / Function Calling**：ChatGPT Plugins（2023.3）→ OpenAI Function Calling（2023.6）→ JSON Schema 标准化 → MCP 协议。核心问题：如何让 LLM 知道"什么时候该用工具、用哪个、怎么填参数"？
- **核心能力二：Planning（规划）**：CoT（2022）→ Tree-of-Thoughts（2023）→ Plan-and-Solve（2023）。核心矛盾：规划能力和执行能力往往是此消彼长的
- **核心能力三：Reflection（反思）**：Self-Refine（2023）→ Reflexion（2023）。关键洞察：Reflection 不是模型架构的能力，而是系统设计的能力
- **核心能力四：Multi-Agent（多智能体协作）**：AutoGen（2023）→ CrewAI（2024）→ 动态 Agent 团队。四种架构：层级式 / 对话式 / 辩论式 / 发布-订阅式

**实践里程碑**：能用"之前有什么问题 → 提出者怎么想的 → 带来了什么变化"的框架讲清楚每种能力；能判断一篇 Agent 论文或产品公告属于哪条能力主线。
...
| **Context Engineering** | 上下文窗口的工程化管理：信息密度优先、位置即优先级、分层管理、性价比思维；Prompt / RAG / Memory / Tool 结果如何在同一窗口里高效编排 | — |
...
**实践里程碑**：将课程三的"裸循环"重构为三层 Harness 架构；构建多步骤 Agent（规划→执行→反思→修正），配有记忆和可观测性；Guardrails 至少拦截 1 次真实的越界行为。
